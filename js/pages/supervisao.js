...
(function () {
  ...
  async function loadData() {
    try {
      ...
      db.rpc('record_health_snapshot', { p_day: todayIso, p_health: state.data.health, p_factors: state.data.factors }).then(({ error: snapshotError }) => { if (snapshotError && snapshotError.code !== 'PGRST202') console.error('[MSY][supervisao] health snapshot:', snapshotError); });
      if (!dailyCheckinDone && state.profile) {
        dailyCheckinDone = true;
        db.from('supervision_timeline').insert({ event_type: 'daily_checkin', title: 'Revisao diaria', actor_id: state.profile.id, occurred_at: new Date().toISOString() }).then(({ error: checkinError }) => { if (checkinError && checkinError.code !== 'PGRST205') console.error('[MSY][supervisao] checkin:', checkinError); });
      }
    } catch (error) {
  ...
  function centralCases() {
    ...
    (d.cases || []).filter((item) => ['open', 'in_progress'].includes(item.status)).forEach((item) => {
      ...
      add({ id: item.id, source: item.source_type, priority: item.priority, title: item.title, description: item.description || 'Caso operacional registrado pela Supervisão.', due: item.due_at, owner: profile?.name || null, memberId: item.member_id, assignedTo: item.assigned_to, route: 'central', persistent: true, status: item.status, raw: item });
    });
